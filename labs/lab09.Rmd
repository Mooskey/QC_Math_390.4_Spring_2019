...
storms %>%
  group_by(status) %>%
  summarize(avg_category = mean(as.numeric(as.character(category))), avg_wind_speed = mean(wind), avg_pressure = mean(pressure), avg__ts_diameters = mean(ts_diameter, na.rm = TRUE), avg__hu_diameters = mean(hu_diameter, na.rm = TRUE) )
```
