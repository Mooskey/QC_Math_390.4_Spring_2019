...
ggplot(mapping = aes(cty)) +
  geom_density(data = mpg %>% filter(trans == "automatic"), fill = "blue", col ="white", alpha = 0.2)+
  geom_density(data = mpg %>% filter(trans == "manual"), fill = "red", col ="white", alpha = 0.2)
...
p2 = ggplot(mpg, aes(x = hwy, y = year)) + 
  geom_count()
...
```{r}
pacman::p_load(ggcorrplot, dplyr)
sapply(mpg, class) == '"ordered" "factor"'
mpg %>%
