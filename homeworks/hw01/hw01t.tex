...
\easysubproblem{What is the difference between \emph{predict} and \emph{forecast}? Are these two terms used interchangably today?}\spc{4}
	...
	In Shakespearian vernacular, prediction referred to the foretelling of fate while forecast referred to foresight and planning under uncertainty. Today,mean largely the same thing.

\easysubproblem{What is John P. Ioannidis's findings and what are its implications?}\spc{5}

	The majority of "successful predictions of medical hypotheses" are false in reality. This implies that for all the data we've accrued and the paradigms we've viewed data through, our future-vision is still pretty dim.
...
\easysubproblem{What are the human being's most powerful defense (according to Silver)? Answer using the language from class.}\spc{4}
	...
	The ability to abstract and approximate reality based on observation and recognizing patterns.

\easysubproblem{Information is increasing at a rapid pace, but what is not increasing?}\spc{3}
	
	Useful information and objective truths
...
\easysubproblem{In a nutshell, what is Karl Popper's (a famous philosopher of science) definition of \emph{science}?}\spc{4}
	...
	According to Karl Popper, science is tests of reality based on falsifiable prediction.
...
\intermediatesubproblem{Why did the ratings agencies say the probability of a CDO defaulting was 0.12\% instead of the 28\% that actually occured? Answer using concepts from class.}\spc{4}
	...
	Because of the large discrepancy between reality and the models procured: rating agencies had an oversimplified model that assumed independence between different mortgages. This assumption failed to take the economic climate into account. Had the agencies done so, they would have seen a more clearly defined area of uncertainty, instead of a haze with a crudely slapped "risk" label on it.

\easysubproblem{What is the difference between \emph{risk} and \emph{uncertainty} according to Silver's definitions?}\spc{4}
	
	A risk is an unknown outcome from a known set of possibilities. Uncertainty is an unknown outcome from an unknown set of possibilities.
	A risk is easily measurable, while uncertainty is much more difficult to calculate.
